...
function WebshotProxy(cfg, fileNameResolver) {
    ...
    if (cfg) {
        ...
        this.snscfg = this.globalCfg.sns_platform;
        if (this.upyuncfg) {
            console.log('init upyun:' + JSON.stringify(this.upyuncfg))
            this.upyun = new UPYun(this.upyuncfg.bucketname, this.upyuncfg.username, this.upyuncfg.password);
        }
        if (this.snscfg) {
            console.log('init weibos:' + JSON.stringify(this.snscfg))
            for (i=0;i<this.snscfg.length;i++) {
                item = this.snscfg[i];
                console.log('######')
                console.log(JSON.stringify(item));
                weibo.init(item.type, item.app_key, item.app_secret, item.oauth_callback_url);
            }
        }
    }
    if (fileNameResolver) {
...
WebshotProxy.prototype.shot2SNS = function(url, user, status, force, cb) {
    var self = this;
    ...
    if (!localpath || force) {
        ...
        this.shot2File(url, null, function (locPath, fName, cfg) {
            filedata = fs.readFileSync(locPath, null);
            weibo.upload(user, status,{data:filedata, name:fName, content_type:'image/'+cfg.proto.streamType}
                , function(err, rst){
                console.log(err, rst);
                if(cb){
                    cb(err, rst);
